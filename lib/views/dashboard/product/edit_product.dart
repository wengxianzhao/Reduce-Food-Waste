...
class _EditProductState extends State<EditProduct> {
  ...
  void _processPostFoodItem() async {
    ...
    if (_category.value == Common.foodCategories.first.value) {
      ...
      Geolocator.getCurrentPosition().then((value) async {
        ...
        String path = widget.foodItem.imageUrl;
        if (_selectedImage != null)
          path = await ApiRequests.uploadSelectedImage(_selectedImage!);
        await ApiRequests.updateFoodItem(
          widget.user.id,
          widget.foodItem.id,
          _foodTitle,
          _category.value,
  ...
  void _bindInitialDetails() {
    _titleController.text = widget.foodItem.title;
    _descriptionController.text = widget.foodItem.description;
    _originalPriceController.text = widget.foodItem.originalPrice.toString();
    _discountedPriceController.text =
        widget.foodItem.discountedPrice.toString();
    Common.foodCategories.forEach((element) {
      if (element.value == widget.foodItem.category) _category = element;
    });
    _addressController.text = widget.foodItem.location.address;
    _address = widget.foodItem.location.address;
    _filePath = widget.foodItem.imageUrl;
    _isLoading = false;
    setState(() {});
  }
}
